...
| I | 讚 | `42` | 程式回寫，不要手動改 |
| J | 跳過 | `7` | 程式回寫，不要手動改 |
| K | 時事摘要 | 主計總處公布最新租金指數，雙北房租連續第三季上漲。 | **只有時事題要填**，一般題目留空 |
| L | 出處 | `https://www.dcard.tw/f/…` | 同上。只收 `http(s)`，其餘一律當作沒有 |
| M | 時事日期 | `2026-08-20` | 同上。決定新鮮度權重與何時停止出牌 |

C、D、E 三欄建議用「資料驗證 → 下拉式選單」，避免打錯字。選項必須與 `pages/deep-talk/taxonomy.js` 完全一致。

K、L、M 是時事題專用的。三欄只要缺一欄，那一題就只是普通題目 —— 抽到的人看不到背景說明，也不會過期。
...
| `generate-news` | 只有你 | 連網找時事並轉成題目，需要 `key` |
...
`deepTalkGenerateNews()` 是另一條路：它會連網。編輯器裡執行 `deepTalkGenerateNewsFromEditor()`，改 `count` 與 `days` 兩個數字就好。

一次請求同時開三樣東西：**搜尋**找出最近被討論的事、**讀網頁**去讀那幾頁的實際內容、**結構化輸出**把結果整理成固定欄位。兩個工具要一起開 —— 只看搜尋摘要寫不出好題目，有味道的東西在留言區不在標題。

生出來的每一題都帶著摘要、出處與時事日期，三者缺一就整題丟掉。

#### 題材黑名單

熱門話題裡永遠混著命案、天災、性犯罪。`deepTalkNewsBlockedWords()` 會擋掉這類題材，回傳值裡的 `blocked` 就是被擋掉的數量。

**這只是第一道，不是最後一道。** 時事題跟一般 AI 題一樣進待審區，你看過才會上架 —— 這道人工關卡對時事題尤其不能放寬，因為題材是模型自己挑的，不是你指定的。

#### 額度

Google Search grounding 是**分開計價**的，不含在一般的 token 費用裡。Gemini 3.x 目前是每月 5,000 次搜尋免費、之後每千次 14 美元。純免費層（沒開帳單）是否享有這個免費額度，官方文件沒有寫清楚，論壇上的官方回答講的是付費 Tier 1 —— 要自己打一次才知道。

實務上這不是問題：一週手動跑一次、一次生 6～12 題，等於一週 1～3 次搜尋請求。真正的瓶頸是你要花多少時間審題。

另外，**免費層送出去的內容 Google 會拿去改進產品，真人也可能看到**。這個用途沒差（生的是要公開的題目），但別拿同一把金鑰去送私人資料。

### 時事題什麼時候會消失

超過 90 天（`DEEPTALK_NEWS_MAX_AGE_DAYS`）就不再出牌。在那之前先有新鮮度權重：半衰期 30 天，一個月前的時事被抽到的機率剩一半，三個月前剩八分之一 —— 到上限生效前其實已經幾乎抽不到，兩道機制是連續的而不是突然斷掉。

權重用的是加權洗牌，不是照日期排序。直接排序會讓最新那一題每次都排第一張。

一般題目沒有時事日期，權重一律 1，行為跟以前完全一樣。
