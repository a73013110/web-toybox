 ...
 *
 * 這一層只管「怎麼送」：endpoint、逾時、JSON、ok 判斷、錯誤代碼。
...
export class AppsScriptClient {
   ...
   * 送出一筆資料並取回結果。
   *
   * ⚠️ body 必須自己 JSON.stringify() 成字串再交給 HttpClient。直接傳物件的話
   *    Angular 會把 Content-Type 蓋成 application/json，瀏覽器就會先送出
   *    Apps Script 不支援的 CORS 預檢，請求直接失敗。
   */
  async send<T>(app: string, payload: Record<string, unknown>): Promise<T> {
    const request = this._http.post<T & AppsScriptEnvelope>(
      ...
      { headers: { 'Content-Type': 'text/plain;charset=utf-8' }, responseType: 'json' }
    );
   ...
   * 在頁面關閉的當下送出資料。
   *
   * sendBeacon 由瀏覽器在頁面卸載後自行完成，一般的 fetch 到這個時機多半會被中斷；
   * 代價是拿不到結果，因此只適合「送不到也無所謂」的補送。
   *
   * 刻意不經過 HttpClient：這是瀏覽器排程的 fire-and-forget，
...
function toAppsScriptError(error: unknown): AppsScriptError {
  if (error instanceof TimeoutError) return new AppsScriptError('timeout');
