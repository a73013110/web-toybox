...
export const invitationSchema = schema<InvitationModel>((path) => {
  validate(path.inviteeName, ({ value }) =>
    normalizeInvitationText(value())
      ? undefined
      : { kind: 'required', message: '請填入姓名或稱呼。' }
  );
  ...
  validate(path.customActivity, ({ value, valueOf }) =>
    !valueOf(path.customEnabled) || normalizeInvitationText(value())
      ? undefined
      : { kind: 'required', message: '請填寫自訂項目。' }
  );
  ...
  validate(path.activities, ({ value, valueOf }) => {
    ...
    const hasCustom =
      valueOf(path.customEnabled) && Boolean(normalizeInvitationText(valueOf(path.customActivity)));
